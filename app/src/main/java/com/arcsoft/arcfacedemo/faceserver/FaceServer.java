...
import com.arcsoft.arcfacedemo.db.manager.RegisterInfoManager;
...
public class FaceServer {
    ...
    private static List<RegisterInfo> sRegisterInfoList;
    ...
    public boolean initRegister(Context context){
        synchronized (this){
            if(faceEngine == null && context != null){
                faceEngine = new FaceEngine();
                int initCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_IMAGE, FaceEngine.ASF_OP_0_HIGHER_EXT,
                        16, 1,
                        FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);
                if(initCode == ErrorInfo.MOK){
                    initRegisterFaceList(context);
                    return true;
                }else{
                    faceEngine = null;
                    Log.e(TAG,"init: failed! code = " + initCode);
                    return false;
                }
            }
            return false;
    ...
    public void unInit() {
        synchronized (this) {
            ...
            if(sRegisterInfoList != null){
                sRegisterInfoList.clear();
                sRegisterInfoList = null;
            }
        }
    }
    ...
    private void initRegisterFaceList(Context context){
        synchronized (this){
            List<RegisterInfo> infoList = RegisterInfoManager.getInstance().getRegisterInfo();
            if(infoList != null && infoList.size() > 0){
                sRegisterInfoList = new ArrayList<>();
                for(RegisterInfo registerInfo : infoList){
                    Log.i(TAG,"info message details = " + registerInfo.getPath());
                    sRegisterInfoList.add(registerInfo);
                }
            }else{
                Toast.makeText(context,context.getString(R.string.rg_list_empty),Toast.LENGTH_SHORT).show();
            }
        }
    }
     ...
     * @return 是否注册成功
     */
    public boolean register(Context context, byte[] nv21, int width, int height, String name,String serial,String age,String sex,int trackId) {
        synchronized (this) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        File file = new File(imgDir + File.separator + imageName + IMG_SUFFIX);
...
//                        FileOutputStream fosFeature = new FileOutputStream(featureDir + File.separator + imageName);
//                        fosFeature.write(faceFeature.getFeatureData());
//                        fosFeature.close();
                        Log.i(TAG,"absolute path :" + file.getAbsolutePath());
                        String imagePath = SAVE_IMG_DIR + File.separator + file.getName();
                        RegisterInfo registerInfo = new RegisterInfo(faceFeature.getFeatureData(),name,age,sex,serial,trackId, file.getAbsolutePath());
                        RegisterInfoManager.getInstance().insertRegisterInfo(registerInfo);
    ...
    public CompareRgResult compareFace(FaceFeature faceFeature){
        if (faceEngine == null || isProcessing || faceFeature == null || sRegisterInfoList == null || sRegisterInfoList.size() == 0) {
            return null;
        }
        FaceFeature tempFaceFeature = new FaceFeature();
        FaceSimilar faceSimilar = new FaceSimilar();
        ...
        int maxSimilarIndex = -1;
        isProcessing = true;
        for (int i = 0; i < sRegisterInfoList.size(); i++) {
            tempFaceFeature.setFeatureData(sRegisterInfoList.get(i).getFeatureDate());
            faceEngine.compareFaceFeature(faceFeature, tempFaceFeature, faceSimilar);
            Log.i(TAG,"SIMILAR VALUE = " +faceSimilar.getScore());
            if (faceSimilar.getScore() > maxSimilar) {
                maxSimilar = faceSimilar.getScore();
                maxSimilarIndex = i;
            }
        ...
        isProcessing = false;
        if (maxSimilarIndex != -1) {
            return new CompareRgResult(sRegisterInfoList.get(maxSimilarIndex), maxSimilar);
        }
        return null;
    }
